...
public class studyRestController3 {
    ...
    body의 특징 -url이 아닌곳에서 데이터를 송신가능 -> 보안이 좋다.
    JSON을 body에 담아서 송신
     */
    ...
    public String test1(@RequestBody Map<String,Object>data){
        //@RequestBody ->body에 작성되어있는 json데이터를 알아서 자바객체로 바꿔준다.
    ...
    //test3라는 이름으로 AddPostReqDto 타입의 데이터를 수신해주세요
    //PostMapping
    @PostMapping("/test3")
    public ResponseEntity<?> test3(@RequestBody AddPostReqDto dto){

        //ResponseEntity:HTTP 응답을 자바에서 커스터마이징하기 편하게 만든 클래스
        //제너릭 타입을 받는다. ->body에 들어가는 데이터 타입
        //HTTP 상태코드, body, header 등을 쉽게 지저할 수 있다.


        //내가 정의한 클래스의 객체로 데이터를 받을 때
        //주의)필드명과 JSON key 이름을 동일하게 해야한다.
        ...
        400: 잘못된요청 BAD_REQUEST, 401:인증 실패 UNAUTHORIZED
    ...
    /*
    어차피 RequestBody로 id를 전달하면 되는거 아닌가?
    -기술적으로는 RequestBody로 id까지 객체로 데이터를 받아서 코딩할 수 있다.
    -RESTful 설계
        -URL;"어떤 자원을 다룰것인가?"(식별) ->id
        -METHOD:"뭘할건가?"(행위에 관한 것)->put(수정)
    ...
    // PUT ->들어온 dto데이터로 싹 다 바꾸겠다는 의미
    // PATCH ->들어온 dto데이터 중 일부만 반영하겠다.

    //DELETE->body 쓸 수 있는데,보통 안 씀
