...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        try {
            ...
            // Authentication을 만들어야한다
            //만들려면  userId/authorities 필요
            String userId=claims.get("sub",String.class);
            String roleName=claims.get("role",String.class);
            //스프링 시큐리티는 권한(role)검사할 때 접두ㅏ("ROLE_")가 필수로 있어야 한다.
            ...
            //SecurityContext(인증저장소)는 여러 사용자들의 인증저장 정보소
            //함부로 접근하지 못하게 SecurityContextHolder라는 심부름꾼을 만들어놓았다.
            //모든 필터들을 SecurityContext를 공유하고 있음.
            //다음 필터에 넘어갈 시점에 authentication을 SecurityContext에 저장했다면 인증이 완료 처리된다.
            SecurityContextHolder.getContext().setAuthentication(authentication);

        } catch(JwtException e){
            //검증 실패시 예외가 발생할 수 있음
            //우리가 예외를 구분해야한다.
            //시간 만료시->ExpireException(JwtException을 상속받고 있음)
            //->특정 에러메세지를  프론트로 내려주고 재요청시킬거임(refresh 컨트롤러로)

            request.setAttribute("exception",e);
            //예외가 전파되지 않게하고 발생한 예외는 request 객체에 담아두자
        }
